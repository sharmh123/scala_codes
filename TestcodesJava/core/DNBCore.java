...
public class DNBCore {
    private static String baseDir = "/Users/himanshusharma/IdeaProjects/resources/DNB";
    ...
    private static double showMathQuestion(MathQuestion mathQuestion, int index) {
        long startTime = System.currentTimeMillis();
        String mathQuestionResponse = JOptionPane.showInputDialog(null,
                mathQuestion.getQuestion(), "Question :" + index,
    ...
    public static double displayMathsQuestion(int difficulty, int index) {
        MathQuestion mathQuestion = new MathQuestion(difficulty).getRandomMathQuestion();
        return showMathQuestion(mathQuestion, index);
    }

    public static double displayMathsQuestion(int index) {
    ...
    public static List<String> readStudyInputFile(String inputFile) {
        List<String> lines = DNBCore.readFile(inputFile);
        //lines = lines.stream().filter(x -> x.trim().isEmpty()).collect(Collectors.toList());
    ...
    public static void displayResults(ConfigContainer configContainer) throws IOException {
        String inputFileName = configContainer.getInputComparisonFile();
        String outputFileName = configContainer.getOutputFileName();
        String outputHistoryFileName = configContainer.getOutputHistoryFileName();
    ...
    private static void showInputDialog(ConfigContainer configContainer, String message, int index,
        ...
        if (!configContainer.isSkimThroughMode()) {
            ...
            try {
                if (configContainer.isPrintOutputAfterEverySentence() && (configContainer.getNback() == 1)) {
                    DNBSentencesCore.printOutput(message, response, configContainer, index);
                }
                bw.write(response + "\r\n");
            } catch (IOException e) {
    ...
    public static void handleInterference(ConfigContainer configContainer) {
        if (configContainer.getInterference()) {
            ...
            for (int count = 1; count <= interferenceLevel; count++) {
                displayMathsQuestion(count);
            }
        }
    }
    ...
    public static void startGameExecution(List<String> exerciseWords, ConfigContainer configContainer) throws IOException {
        if (configContainer.isReverseModeOn()) {
            List<String> reversedWords = exerciseWords.stream()
                    .map(x -> new StringBuilder(x).reverse().toString()).toList();
        }
        ...
        for (String word : exerciseWords) {
            ...
            if (message != null && !message.equals(""))
                JOptionPane.showMessageDialog(null,
                        message,
                        configContainer.getInputSentenceType()
